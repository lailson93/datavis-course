// https://observablehq.com/@lailson93/les-miserables-network@121
  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","dataset","forceSimulation","drag"], function(d3,DOM,dataset,forceSimulation,drag)
{
  ...
  nodes.forEach(function(n){
    teste[n.id] = 0
    // return nodesMap.set(teste[],n)
  })             
  links.forEach(function(l){
    teste[l.source.id] += 1
    teste[l.target.id] += 1
    // nodesMap.get(l.source).id +=1
    // nodesMap.get(l.target).id +=1
  })
  
  let circleRadius = d3.scaleSqrt()
                       .range([3,15])
                       .domain(d3.extent(nodes,d => teste[d.id]))
  ...
  const link = svg.append("g")
   ...
   //Crie os elementos svg para os nodes e guarde-os em node
    const node = svg.append("g")
                  ...
                  .append("circle")
                  .attr("class","node")
                  .attr("r",d => circleRadius(teste[d.id]))
                  .attr("fill", d => colours(teste[d.id]))
                  .call(drag(simulation))
    node.append("title")
        .text(d => d.id +": " +teste[d.id])
  // Defina a função ticked
  function ticked() { //funcao chamada a cada passo da iteracao. link eh uma linha, uma linha visual.
...
function forceSimulation(nodes, links) {
  return d3.forceSimulation(nodes)
           .force("link", d3.forceLink(links).id(d => d.id).distance(50)) //forca aresta positiva: puchar
           .force("charge", d3.forceManyBody().strength(-50).distanceMax(270)) //forca do no: repelir
           .force("center", d3.forceCenter())
}
)});
  main.variable(observer("forceSimulation1")).define("forceSimulation1", ["d3"], function(d3){return(
function forceSimulation1(nodes, links) {
  return d3.forceSimulation(nodes)
